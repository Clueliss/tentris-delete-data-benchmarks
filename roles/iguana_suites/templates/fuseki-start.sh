...
java -Xmx{{ item[1].max_ram }}K -XX:ActiveProcessorCount=120 -jar {{ target_dir }}/triplestores/fuseki/apache-jena-fuseki-{{ fuseki_version }}/fuseki-server.jar --update --tdb2 --loc={{ target_dir }}/databases/fuseki/{{ item[1].name }} /{{ item[1].name }} </dev/null 2>&1 >{{ target_dir }}/logs/run/fuseki-{{ item[1].name }}.log & disown
pid=$!
