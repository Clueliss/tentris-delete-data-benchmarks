...
{{ target_dir }}/triplestores/graphdb/graphdb-free-{{ graphdb_version }}/bin/graphdb -XX:ActiveProcessorCount=1 -d -p {{ target_dir }}/graphdb.pid -s -Dgraphdb.home={{ target_dir }}/databases/graphdb/{{ item[1].name }}
