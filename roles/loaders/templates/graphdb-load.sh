...
cgmemtime "{{ target_dir }}/triplestores/graphdb/graphdb-{{ graphdb_version }}/bin/importrdf" preload -f -c {{ target_dir }}/triplestores/graphdb/graphdb-{{ item[1].name }}.ttl {{ item[1].path }} 2>&1 | tee {{ target_dir }}/logs/load/graphdb-{{ item[1].name }}.log
